      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-zinc-200 shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-755 disabled:bg-zinc-100 disabled:text-zinc-400 disabled:cursor-not-allowed text-white font-bold py-2.5 px-4 text-xs transition-all active:scale-97 shadow-sm shadow-indigo-600/10 cursor-pointer"
          >
            {iframeLoaded ? <Printer className="h-4 w-4" /> : <Loader2 className="h-4 w-4 animate-spin" />}
            Print Document
          </button>
        ...
        {!iframeLoaded ? (
          <div className="bg-indigo-50/50 border-b border-indigo-100 px-4 py-3 sm:px-6 flex items-start gap-2.5 text-indigo-900">
            ...
            <div className="text-left">
              <p className="text-xs font-bold leading-tight">Rendering document preview...</p>
              <p className="text-[11px] text-zinc-500 font-medium mt-0.5 leading-normal">
                Please wait while we render your document preview from the secure database.
              </p>
            </div>
          </div>
        ) : (
          <div className="bg-emerald-50/50 border-b border-emerald-100 px-4 py-3 sm:px-6 flex items-start gap-2.5 text-emerald-900 animate-fade-in">
            <CheckCircle className="h-4.5 w-4.5 text-emerald-600 shrink-0 mt-0.5" />
            <div className="text-left">
              <p className="text-xs font-bold leading-tight">Document preview loaded successfully.</p>
              <p className="text-[11px] text-zinc-500 font-medium mt-0.5 leading-normal">
                Use the <strong>Print Document</strong> button above to print your document or save it locally as a PDF.
              </p>
            </div>
          </div>
        )}
