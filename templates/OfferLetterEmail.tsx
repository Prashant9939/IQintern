...
export function getOfferLetterEmailHtml(fullName: string, internshipTitle: string): string {
  const content = `
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${BRANDING.websiteUrl}/student/documents" target="_blank" rel="noopener noreferrer" style="background-color: #4F46E5; color: #FFFFFF; text-decoration: none; padding: 12px 28px; border-radius: 9999px; font-size: 14px; font-weight: 600; display: inline-block;">
        View Documents
      </a>
    </div>
  `;
  return getEmailLayout(`Offer Letter | ${BRANDING.name}`, content);
}
